...
class TypeRefinementListener extends riddleBaseListener {
    ...
    public void enterEnum_declaration(riddleParser.Enum_declarationContext ctx) {
        ...
        ctx.enum_constants().stream().filter(enum_constant -> (enum_constant.type() != null)).forEach(
                enum_constant -> c_type.enums.add((EnumType) new TypeVisitor(core).visit(enum_constant.type())));
    }
