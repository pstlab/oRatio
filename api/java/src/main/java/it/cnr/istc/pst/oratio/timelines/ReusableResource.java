...
import it.cnr.istc.pst.oratio.riddle.Item.ArithItem;
...
public class ReusableResource implements Timeline<ReusableResource.RRValue> {
    ...
    private static class ReusableResourceBuilder implements TimelineBuilder {
        ...
        public ReusableResource build(Item itm, Collection<Atom> atoms) {
            ...
            for (Atom atom : atoms) {
                InfRational start_pulse = ((Item.ArithItem) atom.getExpr(Core.START)).getValue();
                InfRational end_pulse = ((Item.ArithItem) atom.getExpr(Core.END)).getValue();
