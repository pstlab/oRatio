...
namespace ratio
{
  ...
  public:
    ...
    EXECUTOR_EXPORT const smt::rational &get_current_time() const { return current_time; };
    EXECUTOR_EXPORT const smt::rational &get_units_per_tick() const { return units_per_tick; };
  ...
  private:
    ...
    std::unordered_map<const atom *, smt::rational> dont_start;             // the starting atoms which are not yet ready to start..
    std::unordered_map<const atom *, smt::rational> dont_end;               // the ending atoms which are not yet ready to end..
