...
private:
  ...
  bool check_flaw(flaw &f, std::unordered_set<flaw *> &visited, std::unordered_set<resolver *> &inv_rs, std::unordered_set<resolver *> &inc_rs, std::vector<refinement_flaw *> &ref_fs, std::unordered_map<resolver *, std::vector<resolver *>> &to_merge); // checks the solvability of the given flaw filling..
#endif
