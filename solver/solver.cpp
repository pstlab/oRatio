...
main_loop:
	...
	while (!_layers.empty()) {
		...
		if (!push(least_expensive_choice)) {
			...
			bool added = _net.add({ _net.disjunction(no_good) });
			assert(added);
			goto main_loop;
		}
...
bool solver::push(choice * const ch) {
	...
	return _net.assign_true(ch->_in_plan);
}
