...
{
    assert(slv.sat_cr.value(get_phi()) != smt::False);
    assert(slv.sat_cr.value(atm.sigma) != smt::False);
    if (slv.sat_cr.value(atm.sigma) == smt::Undefined) // we check if the atom can unify..
    {
        ...
        while (!q.empty())
        {
            if (ancestors.find(q.front()) == ancestors.end())
            {
                ...
                for (const auto &supp : q.front()->get_supports())
                    if (slv.sat_cr.value(supp->get_rho()) != smt::False) // if false, the edge is broken..
        ...
        for (const auto &i : atm.tp.get_instances())
        {
            ...
            if (!target.is_expanded() ||                       // the target flaw must hav been already expanded..
                ...
                slv.sat_cr.value(c_atm.sigma) == smt::False || // the target atom is unified with some other atom..
            ...
            q.push(this);
            q.push(&target);
            unif_lits.push_back(smt::lit(atm.sigma, false)); // we force the state of this atom to be 'unified' within the unification literals..
            ...
            if (slv.sat_cr.value(eq_v) != smt::True)
...
void atom_flaw::activate_fact::apply() { slv.sat_cr.new_clause({smt::lit(rho, false), atm.sigma}); }
...
{
    slv.sat_cr.new_clause({smt::lit(rho, false), atm.sigma});
    static_cast<const predicate &>(atm.tp).apply_rule(atm);
}

atom_flaw::unify_atom::unify_atom(solver &slv, atom_flaw &f, atom &atm, atom &trgt, const std::vector<smt::lit> &unif_lits) : resolver(slv, 1, f), atm(atm), trgt(trgt), unif_lits(unif_lits) {}
atom_flaw::unify_atom::~unify_atom() {}

void atom_flaw::unify_atom::apply()
{
    for (const auto &v : unif_lits)
        slv.sat_cr.new_clause({smt::lit(rho, false), v});
}
}