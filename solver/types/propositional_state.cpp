...
namespace ratio
{
    ...
    {
        new_constructors({new ps_constructor(*this)});    // we add a constructor..
        new_predicates({new ps_predicate(*this)}, false); // we add the 'PropositionalStatePredicate' predicate, without notifying neither the resource nor its supertypes..
    }
