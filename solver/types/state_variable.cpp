...
namespace ratio
{
                ...
                if (var_item *enum_scope = dynamic_cast<var_item *>(&*c_scope))
                {
                    for (const auto &val : get_core().get_ov_theory().value(enum_scope->ev))
                        if (to_check.count(static_cast<const item *>(val)))
                            sv_instances[static_cast<const item *>(val)].emplace_back(atm);
                }
                else if (to_check.count(static_cast<item *>(&*c_scope)))
                    sv_instances[static_cast<item *>(&*c_scope)].emplace_back(atm);
            }
    ...
    std::string state_variable::sv_flaw::get_data() const
    {
    ...
    state_variable::order_resolver::order_resolver(sv_flaw &flw, const lit &r, const atom &before, const atom &after) : resolver(r, rational::ZERO, flw), before(before), after(after) {}
    ...
    state_variable::place_resolver::place_resolver(sv_flaw &flw, const lit &r, atom &plc_atm, const item &plc_itm, atom &frbd_atm) : resolver(r, rational::ZERO, flw), plc_atm(plc_atm), plc_itm(plc_itm), frbd_atm(frbd_atm) {}
    ...
    state_variable::forbid_resolver::forbid_resolver(sv_flaw &flw, atom &atm, item &itm) : resolver(rational::ZERO, flw), atm(atm), itm(itm) {}
