...
std::vector<flaw *> state_variable::get_flaws()
{
    ...
    else
    {
        ...
        for (const auto &atm : atoms)
            if (get_solver().get_sat_core().value(atm.first->get_sigma()) == True) // we filter out those which are not strictly active..
            {
...
void state_variable::new_predicate(predicate &pred)
{
    ...
    new_supertypes(pred, {&get_solver().get_predicate("IntervalPredicate")});
...
void state_variable::new_fact(atom_flaw &f)
{
    ...
    get_solver().get_predicate("IntervalPredicate").apply_rule(atm);
    restore_ni();
